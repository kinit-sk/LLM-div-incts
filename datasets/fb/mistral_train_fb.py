...
sample_template = r'Text: "{}". Question: What is the intent of this text based on options: "Directions", "Distance", "Arrival", "Departure", "Duration", "Road Condition", "Route", "Traffic", "Location", "Update"?'
label_template = r'Answer: {}'
dct_labels = {'0': 'Directions', '1': 'Distance', '2': 'Arrival', '3': 'Departure', '4':'Duration', '5':'Road Condition', '6':'Route', '7':'Traffic', '8':'Location', '9':'Update'}

dct_rev_labels = {'Directions':0, 'Distance':1, 'Arrival':2, 'Departure':3, 'Duration':4, 'Road Condition':5, 'Route':6, 'Traffic':7, 'Location':8, 'Update':9}

os.environ["CUDA_VISIBLE_DEVICES"] = "0"
os.environ['CURL_CA_BUNDLE'] = ''
MODEL_NAME = "mistralai/Mistral-7B-v0.1"
new_model = 'Mistral-7B-fb'
...
for iteration in range(0,3):
    for model_type in ['gpt4', 'gpt', 'llama', 'play', 'mistral']:
        for method in ['prompt', 'taboo', 'hints', 'chaining', 'taboo_ablt', 'hints_ablt', 'chaining_ablt', 'comb']:
            for i in range(0,5):
                print('fb_under/'+model_type+'/'+str(i)+'/fb_'+method+'.csv')
                df = pd.read_csv('fb_under/'+model_type+'/'+str(i)+'/fb_'+method+'.csv')
                chat_samples = []
                labels = []
                ...
                df = pd.DataFrame.from_dict(dct_df).to_csv('fb_under/'+model_type+'/'+str(i)+'/train_fb_mistral_'+method+'.csv',index=False)
                ...
                df = pd.read_csv('fb_under/'+model_type+'/'+str(i)+'/train_fb_mistral_'+method+'.csv')
                ...
                df_test = pd.read_csv('fb_under/full_fb_orig_test.csv').sample(frac=0.1, random_state=1)
                ...
                df_test.to_csv('fb_under/'+model_type+'/'+str(i)+'/mistral_ft_res_'+method+'_'+str(iteration)+'.csv',index=False)