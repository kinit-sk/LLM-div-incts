...
def train_loop(model, ds, BATCH_SIZE=64, NUM_EPOCHS=80):
    ...
    df_test = pd.read_csv('fb/full_fb_orig_test.csv')
...
for model_name in ['mistral']:
    for method in ['prompt', 'taboo', 'chaining', 'hints']:
        for iteration in range(0,5):
            ...
            accs = []
            for i in range(0,5):
                df = pd.read_csv('fb/'+model_name+'/'+str(iteration)+'/fb_'+str(method)+'.csv')
                ds = prepare_data(df)
                ...
                with open('fb/'+model_name+'/'+str(iteration)+'/history_'+str(method)+'_'+str(i)+'.pkl', 'wb') as handle:
                    pickle.dump(history_cart_dct, handle)
                    
            with open('fb/'+model_name+'/'+str(iteration)+'/'+str(method)+'_accs_experiment.pkl', 'wb') as handle:
