...
sample_template = r'Text: "{}". Question: How many stars does this text have based on options: "one star", "two stars", "three stars", "four stars", "five stars"?'
label_template = r'Answer: {}'

dct_labels = {'0': 'one star', '1': 'two stars', '2': 'three stars', '3': 'four stars', '4':'five stars'}

dct_rev_labels = {'one star':0, 'two stars':1, 'three stars':2, 'four stars':3, 'five stars':4}

os.environ["CUDA_VISIBLE_DEVICES"] = "0"
os.environ['CURL_CA_BUNDLE'] = ''
MODEL_NAME = "mistralai/Mistral-7B-v0.1"
new_model = 'Mistral-7B-yelp'
...
for iteration in range(0,3):
    for model_type in ['gpt', 'gpt4', 'llama', 'play', 'mistral']:
        for method in ['prompt', 'taboo', 'hints', 'chaining', 'comb', 'taboo_ablt', 'hints_ablt', 'chaining_ablt']:
            for i in range(0,5):
                print('yelp/'+model_type+'/'+str(i)+'/yelp_'+method+'.csv')
                df = pd.read_csv('yelp/'+model_type+'/'+str(i)+'/yelp_'+method+'.csv')
                chat_samples = []
                labels = []
                ...
                df = pd.DataFrame.from_dict(dct_df).to_csv('yelp/'+model_type+'/'+str(i)+'/train_yelp_mistral_'+method+'.csv',index=False)
                ...
                df = pd.read_csv('yelp/'+model_type+'/'+str(i)+'/train_yelp_mistral_'+method+'.csv')
                ...
                df_test = pd.read_csv('yelp/test_yelp.csv').sample(frac=0.1, random_state=1)
                ...
                df_test.to_csv('yelp/'+model_type+'/'+str(i)+'/mistral_ft_res_'+method+'_'+str(iteration)+'.csv',index=False)