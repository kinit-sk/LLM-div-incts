...
df_train = pd.read_csv('yelp/train_yelp.csv')
...
for NO_TRY in range(4,5):
    N_SAMPLES = 6
    
    get_subsampled = df_train.groupby('label', group_keys=False).apply(lambda x: x.sample(N_SAMPLES))

    get_subsampled.to_csv('yelp/llama/'+str(NO_TRY)+'/seeds.csv', index=False)
    dct_phrases = {}
    ...
    fb_0.drop_duplicates().to_csv('yelp/llama/'+str(NO_TRY)+'/fb_0_seeds.csv', index=False)

    dct_responses = collect_samples(dct_final_prompts)

    fb_0 = filter_responses(dct_responses)
    fb_0.drop_duplicates().to_csv('yelp/llama/'+str(NO_TRY)+'/fb_1_seeds.csv', index=False)

    fb_0 = pd.read_csv('yelp/llama/'+str(NO_TRY)+'/fb_0_seeds.csv')
    ...
    fb_0 = filter_responses(dct_responses)
    fb_0.drop_duplicates().to_csv('yelp/llama/'+str(NO_TRY)+'/fb_1_taboo.csv', index=False)

    fb_0 = pd.read_csv('yelp/llama/'+str(NO_TRY)+'/fb_0_seeds.csv').drop_duplicates()
    fb_0['text'] = fb_0['text'].astype("string")
    dct_phrases = fb_0.groupby('label')['seed'].apply(set).to_dict()
    ...
    fb_0 = filter_responses(dct_responses)
    fb_0.drop_duplicates().to_csv('yelp/llama/'+str(NO_TRY)+'/fb_1_chaining.csv', index=False)
    ...
    fb_0 = filter_responses(dct_responses)
    fb_0.drop_duplicates().to_csv('yelp/llama/'+str(NO_TRY)+'/fb_1_hints.csv', index=False)
