...
def train_loop(model, ds, BATCH_SIZE=64, NUM_EPOCHS=80):
    ...
    df_test = pd.read_csv('atis/atis_test.csv')
...
for model_name in ['mistral']:
    for method in ['prompt', 'taboo', 'chaining', 'hints']:
        for iteration in range(0,5):
            ...
            for i in range(0,5):
                print(method)
                df = pd.read_csv('atis/'+model_name+'/'+str(iteration)+'/atis_'+str(method)+'.csv')
                ds = prepare_data(df)

                model = get_new_model()
                model, history_cart_dct, acc_tests = train_loop(model, ds)
                df_test = pd.read_csv('atis/atis_test.csv')
                ds = prepare_data(df_test)
                acc = eval_loop(model, ds)
                accs.append(acc_tests)

                with open('atis/'+model_name+'/'+str(iteration)+'/history_'+str(method)+'_'+str(i)+'.pkl', 'wb') as handle:
                    pickle.dump(history_cart_dct, handle)
                    
            with open('atis/'+model_name+'/'+str(iteration)+'/'+str(method)+'_accs_experiment.pkl', 'wb') as handle:
