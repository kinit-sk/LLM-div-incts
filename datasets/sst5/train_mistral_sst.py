...
sample_template = r'Text: "{}". Question: Is this text "very negative", "negative", "neutral", "positive" or "very positive"?'
label_template = r'Answer: {}'

dct_labels = {'0': 'very negative', '1': 'negative', '2': 'neutral', '3': 'positive', '4':'very positive'}

dct_rev_labels = {'very negative':0, 'negative':1, 'neutral':2, 'positive':3, 'very positive':4}
...
for iteration in range(0,3):
    for method in ['prompt', 'hints', 'chaining', 'taboo', 'comb', 'hints_ablt', 'chaining_ablt', 'taboo_ablt']:
        for i in range(0,5):
            df = pd.read_csv('sst5/'+model_type+'/'+str(i)+'/sst_'+method+'.csv')
            chat_samples = []
            labels = []
            ...
            df = pd.DataFrame.from_dict(dct_df).to_csv('sst5/'+model_type+'/'+str(i)+'/train_sst_mistral_'+method+'_'+str(iteration)+'.csv',index=False)
            ...
            df = pd.read_csv('sst5/'+model_type+'/'+str(i)+'/train_sst_mistral_'+method+'_'+str(iteration)+'.csv')
            ...
            df_test = pd.read_csv('sst5/test_sst.csv').sample(frac=0.1, random_state=1)
            ...
            df_test.to_csv('sst5/'+model_type+'/'+str(i)+'/mistral_ft_res_'+method+'_'+str(iteration)+'.csv',index=False)