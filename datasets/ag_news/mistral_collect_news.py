...
df_train = pd.read_csv('ag_news/train_news.csv')
...
def get_taboo_w_for_df_no_ner(df_orig, no_taboo_w, seed_samples_dct):
    ...
    for label in labels:
        ...
        sub_df_orig = sub_df_orig.dropna(subset=['text'])
        ...
        for responses in dct_responses[key]:
            for response in responses[0]:
                ...
                for content in contents:
                    ...
                    dct_df['text'].append(content.replace('inst', ""))
...
for NO_TRY in range(0,5):

    N_SAMPLES = 6
    get_subsampled = df_train.groupby('label', group_keys=False).apply(lambda x: x.sample(N_SAMPLES))
    get_subsampled.to_csv('ag_news/mistral/'+str(NO_TRY)+'/seeds.csv', index=False)
    ...
    fb_0.drop_duplicates().to_csv('ag_news/mistral/'+str(NO_TRY)+'/fb_0_seeds.csv', index=False)

    dct_responses = collect_samples(dct_final_prompts)

    fb_0 = filter_responses(dct_responses)
    fb_0.drop_duplicates().to_csv('ag_news/mistral/'+str(NO_TRY)+'/fb_1_seeds.csv', index=False)

    fb_0 = pd.read_csv('ag_news/mistral/'+str(NO_TRY)+'/fb_0_seeds.csv')
    ...
    fb_0 = filter_responses(dct_responses)
    fb_0.drop_duplicates().to_csv('ag_news/mistral/'+str(NO_TRY)+'/fb_1_taboo.csv', index=False)

    fb_0 = pd.read_csv('ag_news/mistral/'+str(NO_TRY)+'/fb_0_seeds.csv').drop_duplicates()
    fb_0['text'] = fb_0['text'].astype("string")
    dct_phrases = fb_0.groupby('label')['seed'].apply(set).to_dict()
    ...
    fb_0 = filter_responses(dct_responses)
    fb_0.drop_duplicates().to_csv('ag_news/mistral/'+str(NO_TRY)+'/fb_1_chaining.csv', index=False)
    ...
    fb_0 = filter_responses(dct_responses)
    fb_0.drop_duplicates().to_csv('ag_news/mistral/'+str(NO_TRY)+'/fb_1_hints.csv', index=False)
