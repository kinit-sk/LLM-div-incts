...
sample_template = r'Text: "{}". Question: What is the category of this news text based on options: "World", "Sports", "Business", "Science and Technology"?'
label_template = r'Answer: {}'
dct_labels = {'0': 'World', '1': 'Sports', '2': 'Business', '3': 'Science and Technology'}

dct_rev_labels = {'World':0, 'Sports':1, 'Business':2, 'Science and Technology':3}
...
for iteration in range(0,3):
    for method in ['prompt','taboo','hints','chaining', 'comb', 'taboo_ablt','hints_ablt','chaining_ablt']:
        for i in range(0,5):
	    try:
            	df = pd.read_csv('ag_news/'+model_type+'/'+str(i)+'/news_'+method+'.csv')
            except:
            	df = pd.read_csv('ag_news/'+model_type+'/'+str(i)+'/fb_'+method+'.csv')
            chat_samples = []
            labels = []
            ...
            df = pd.DataFrame.from_dict(dct_df).to_csv('ag_news/'+model_type+'/'+str(i)+'/train_agn_mistral_'+method+'.csv',index=False)
            ...
            df = pd.read_csv('ag_news/'+model_type+'/'+str(i)+'/train_agn_mistral_'+method+'.csv')
            ...
            df_test = pd.read_csv('ag_news/test_news.csv').sample(frac=0.1, random_state=1)
            ...
            df_test.to_csv('ag_news/'+model_type+'/'+str(i)+'/mistral_ft_res_'+method+'_'+str(iteration)+'.csv',index=False)