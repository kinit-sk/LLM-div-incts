...
for model_name in ['mistral']:
    for method in ['taboo', 'chaining', 'hints']:
        for iteration in range(0,5):
            ...
            accs = []
            for i in range(0,5):
                df = pd.read_csv('ag_news/'+model_name+'/'+str(iteration)+'/ag_news_'+str(method)+'_ablt.csv')
                ds = prepare_data(df)
            ...
            with open('ag_news/'+model_name+'/'+str(iteration)+'/'+str(method)+'_accs_ablt.pkl', 'wb') as handle:
