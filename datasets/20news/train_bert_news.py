...
def eval_loop(model, ds):
    model.eval()
    test_loader = DataLoader(ds, batch_size=64, shuffle=False)
    with torch.no_grad():
        total_correct = 0
...
def train_loop(model, ds, BATCH_SIZE=32, NUM_EPOCHS=80):
    ...
    df_test = pd.read_csv('20news/test_news.csv')
...
for model_name in ['mistral']: # replace with model name, just make sure the dataset files have the same names
    for method in ['prompt', 'taboo', 'chaining', 'hints']:
        for iteration in range(0,5):

            accs = []
            for i in range(0,5):
                df = pd.read_csv('20news/'+model_name+'/'+str(iteration)+'/20news_'+str(method)+'.csv')
                ds = prepare_data(df)
                ...
                with open('20news/'+model_name+'/'+str(iteration)+'/history_'+str(method)+'_'+str(i)+'.pkl', 'wb') as handle:
                    pickle.dump(history_cart_dct, handle)
                    
            with open('20news/'+model_name+'/'+str(iteration)+'/'+str(method)+'_accs_experiment.pkl', 'wb') as handle:
