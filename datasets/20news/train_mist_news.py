...
parser.add_argument("--model", type=str, default='gpt', choices=['gpt', 'gpt4', 'llama', 'play', 'mistral'])
...
sample_template = r'Text: "{}". Question: What is the category of this news text based on options: "Entertainment", "Parenting", "Politics", "Style and Beauty", "Travel", "Wellness"?'
label_template = r'Answer: {}'
dct_labels = {'0': 'Entertainment', '1': 'Parenting', '2': 'Politics', '3': 'Style and Beauty', '4':'Travel', '5':'Wellness'}

dct_rev_labels = {'Entertainment':0, 'Parenting':1, 'Politics':2, 'Style and Beauty':3, 'Travel':4, 'Wellness':5}
...
for iteration in range(0,3):
    for method in ['prompt', 'hints', 'chaining', 'taboo', 'comb', 'hints_ablt', 'chaining_ablt', 'taboo_ablt']:
        for i in range(0,5):
	    try:
            	df = pd.read_csv('20news/'+model_type+'/'+str(i)+'/fb_'+method+'.csv')
            except:
                df = pd.read_csv('20news/'+model_type+'/'+str(i)+'/news_'+method+'.csv')
            chat_samples = []
            labels = []
            ...
            df = pd.DataFrame.from_dict(dct_df).to_csv('20news/'+model_type+'/'+str(i)+'/train_20n_mistral_'+method+'.csv',index=False)
            ...
            df = pd.read_csv('20news/'+model_type+'/'+str(i)+'/train_20n_mistral_'+method+'.csv')
            ...
            df_test = pd.read_csv('20news/test_news.csv').sample(frac=0.1, random_state=1)
            ...
            df_test.to_csv('20news/'+model_type+'/'+str(i)+'/mistral_ft_res_'+method+'_'+str(iteration)+'.csv',index=False)